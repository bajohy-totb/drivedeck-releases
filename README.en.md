...
**Built-in connection preview:** [1.0.1-rc4 APK](https://github.com/bajohy-totb/drivedeck-releases/releases/download/v1.0.1-rc4/DriveDeck-1.0.1-rc4.apk) connects directly through **wireless debugging without installing Shizuku** and improves physical keyboard input in App 1. The sidebar and favorites bar accept touch without taking keyboard focus. Pair once by entering Android's six-digit code in the DriveDeck notification. Root and Shizuku remain available. Requires Android 13+; the user's phone and vehicle remain unverified. [Setup instructions](docs/guide.en.md#built-in-connection-preview-101-rc4)
...
1. Use an **Android 13 or newer device**. Stable 1.0.0 uses root; preview 1.0.1-rc4 supports built-in wireless debugging, Root and Shizuku. Compatibility depends on the manufacturer's display and input implementation.
...
Automatic checks run when the launcher starts or resumes, skipping checks within six hours of the last one. Downloads and Android installation confirmation remain manual. The stable channel stays on 1.0.0. Enable **Receive preview versions** for 1.0.1-rc4. Park before installing.
