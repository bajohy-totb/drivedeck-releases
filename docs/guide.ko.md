...
이 설명서의 화면과 기본 조작은 **정식 1.1.0** 기준입니다. 시험 버전의 앱별 배율·스크롤 목록·새 홈 조합 편집은 [1.1.1-rc2 안내](preview.ko.md)를 참고하세요. 시험 버전에서는 홈 조합을 한 번 눌러 열고 길게 눌러 기본 홈을 실행합니다.

## 1. 시작하기

DriveDeck 1.1.0은 Android 13 이상에서 자체 무선 디버깅·루트·Shizuku 연결을 지원합니다. APK를 설치하고 기기에 맞는 연결 방식을 설정한 뒤 내비게이션과 보조 앱을 선택하세요. 같은 앱을 두 구역에 동시에 지정할 수 없습니다.
...
실제 폰·차량의 화면 및 입력 호환성은 [검증 범위](validation.md)를 확인하세요. 포함 라이브러리의 고지, 소스와 교체용 자료는 [동일 버전 배포의 relink ZIP](https://github.com/bajohy-totb/drivedeck-releases/releases/tag/v1.1.0)에 제공합니다.
...
| 앱 선택 | 대상 구역을 고르고 분류·검색·페이지 이동으로 앱을 찾습니다. |
...
- **화면:** 자동·좌우·위아래 분할, 30~70% 비율, 순서 뒤집기, 조작 바 자동·왼쪽·오른쪽·아래, 앱·키보드 배율 120~240dpi를 설정합니다. 자동은 시작 시 화면 형태에 맞춥니다. 실행 중 기기 회전은 고정하지만 분할 방향과 바 위치는 바꿀 수 있습니다.
...
홈 버튼을 길게 누르거나 **설정 → 앱 → 앱 조합**에서 이름을 붙인 앱 조합을 최대 8개 저장합니다. 앱 두 개와 비율·분할 방향·순서를 복원합니다. 같은 이름으로 저장하면 교체하며 8개를 넘으면 가장 오래된 항목을 지웁니다. 저장 항목을 길게 누르면 삭제를 확인합니다. 홈 버튼의 기본 조합은 따로 저장·해제할 수 있습니다.

앱 목록은 왼쪽에서 전체·즐겨찾기·최근·내비게이션·음악/영상·기타를 선택하고 오른쪽에서 검색합니다. 화면에 맞춰 아이콘 칸 수가 달라지며, 하단 화살표로 다음 페이지를 엽니다. 각 앱의 ⋯ 또는 길게 누르기로 옵션을 엽니다.
...
**설정 → 업데이트 → 업데이트**에서 새 버전을 확인하고 다운로드·설치합니다. 정식 채널은 1.1.0, **시험 버전 받기**를 켠 채널은 1.1.1-rc2를 제공합니다. RC7 이전 버전은 APK를 한 번 직접 설치해야 합니다. 채널을 바꿔도 구버전으로 자동 내려가지는 않습니다.
