...
DriveDeck 1.0.1은 Android 13 이상에서 자체 무선 디버깅·루트·Shizuku 연결을 지원합니다. APK를 설치하고 기기에 맞는 연결 방식을 설정한 뒤 내비게이션과 앱 1을 선택하세요. 같은 앱을 두 구역에 동시에 지정할 수 없습니다.
...
실제 폰·차량의 화면 및 입력 호환성은 [검증 범위](validation.md)를 확인하세요. 포함 라이브러리의 고지, 소스와 교체용 자료는 [동일 버전 배포의 relink ZIP](https://github.com/bajohy-totb/drivedeck-releases/releases/tag/v1.0.1)에 제공합니다.
...
**설정 → 기기 → 업데이트**에서 새 버전을 확인하고 다운로드·설치합니다. 정식·시험 채널 모두 이번 1.0.1 정식을 받을 수 있습니다. **시험 버전 받기**는 앞으로 공개되는 RC도 받을지 선택하는 설정입니다. RC7 이전 버전은 APK를 한 번 직접 설치해야 합니다. 채널을 바꿔도 구버전으로 자동 내려가지는 않습니다.
